...
class Logging(ServerModule):
    def __init__(self, requests=False, show_event_loggers=False):
        self.show_event_loggers = show_event_loggers
    ...
    async def calculate_send_length(self, pkts):
        length = 0
        for pkt in pkts:
            length += len(await asyncio.to_thread(yaml.safe_dump,pkt))
    ...
    def on_connect(self, ctx):
        if hasattr(ctx.app,'is_event_socket') and ctx.app.is_event_socket(ctx.addr):
            if self.show_event_loggers is False:
                return
            return logger.info(
    ...
    def on_client_error(self, ctx):
        logger.error(
            f"Client error {self.form_addr(ctx.addr)}: {ctx.wrapped_error} {ctx._dict.get('pkt','')}"
        )
    ...
    @event()
    async def on_request_processed(self, ctx):
        elapsed_time = round((time.time() - ctx.pkt_time) * 1000, 3)
        size = await self.calculate_send_length(ctx.responses)
        recv_s = self.format_file_size(len(yaml.safe_dump(ctx.raw_pkt)))
        logger.info(
            f'{ctx.app.form_addr(ctx.addr)} /{ctx.pkt.get("path")} {elapsed_time}ms received {recv_s} sent {size}'
        )

    @event()
    def shutdown_server(self,ctx):
        logger.debug('Shutting down server')
    
    @event()
    def server_closed(self,ctx):
        logger.important_log('Server closed')